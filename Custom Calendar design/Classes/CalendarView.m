...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        _isAllowedEventCreation = NO;
        _eventCreationColor = [UIColor yellowColor];
        ...
        _selectedDateEvents = [[NSMutableArray alloc]init];
    }
    return self;
}
...
-(void)performViewAnimation:(UIViewAnimationOptions)animation
{
    ...
    [_delegate selectedDate:clickedDate];
...
-(void)configureEventCreationButton:(UIButton *)button withDate:(NSDate*)date{

    
    // Unselected button
    button.layer.borderWidth = _borderWidth/2.f;
    DayCellConfigure *config = [_datasource dataForDate:date];
 
    if ( config.subtitle != nil) {
        return;
    }
    
    if ([_selectedDateEvents containsObject: date]) {
        // remove from array .
        [_selectedDateEvents removeObject:date];
        //// Again Declear because of after removing the data get updated result
        DayCellConfigure *config = [_datasource dataForDate:date];
        [button setTitleColor:config.cellBtnTextColor forState:UIControlStateNormal];
        [button setBackgroundColor: config.cellBackgroundColor];
        
        return;
    }
    
    {
    [_selectedDateEvents addObject:date];
    [button setTitleColor:config.cellBtnTextColor forState:UIControlStateNormal];
    [button setBackgroundColor: _eventCreationColor];
    }
...
-(void)configureDayButton:(UIButton *)button withDate:(NSDate*)date
{
    ...
    button.titleEdgeInsets = UIEdgeInsetsMake(-3, 10, 20, -5);
    button.titleLabel.textAlignment = NSTextAlignmentRight;
 
    if([_selectedDate compare:date] == NSOrderedSame && !_isAllowedEventCreation)
    {
        // Selected button
    ...
    else
    {
        ...
        if (config.subtitle != nil) {
            ...
            [formatter setDateFormat:@"dd"];
                ...
                UILabel *eventLabel = [[UILabel alloc]initWithFrame:CGRectMake(8,button.frame.size.height/2-2 , button.bounds.size.width-10, button.frame.size.height/2 )];

                 eventLabel.textAlignment = NSTextAlignmentLeft;
                [eventLabel setTextColor:RGBCOLOR(125, 125, 125)];
                //[eventLabel setBackgroundColor:[UIColor yellowColor]];
                [eventLabel setFont:[UIFont fontWithName: @"Trebuchet MS" size: 11.0f]];
                [eventLabel setText:config.subtitle];
...
-(IBAction)tappedDate:(UIButton *)sender
{
    ...
    NSDateComponents * componentsDateSel = [_gregorian components:_dayInfoUnits fromDate:_selectedDate];
        ...
        // Event Creation after tap cell
        if (_isAllowedEventCreation) {
            // Configure  the new selected day button
            [self configureEventCreationButton:sender             withDate:_selectedDate];
...
@implementation DayCellConfigure
...
-(instancetype)init{
    ...
    if (self) {
        ...
        self.subtitle = nil;
        self.cellBtnTextColor = [UIColor clearColor];
